...
class LoggerInterceptor @JvmOverloads constructor(
    ...
    private fun bodyIsBinary(response: Response): Boolean {
        val contentType = response.body.contentType()
        return contentType != null && (
                binaryType.contains(contentType.type.lowercase())
                        || binaryType.contains(contentType.subtype.lowercase())
                )
    }

    companion object {
        private val binaryType = listOf("video", "audio", "image", "octet-stream", "zip", "pdf")

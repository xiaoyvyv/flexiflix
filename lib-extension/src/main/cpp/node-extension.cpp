...
jint startNodejs(JNIEnv *env, jobjectArray arguments);
...
jint startNodejs(JNIEnv *env, jobjectArray arguments) {//argc
    ...
    __android_log_write(ANDROID_LOG_ERROR, ADBTAG, "start!");

    //Start node, with argc and argv.
    int node_result = node::Start(argument_count, argv);
    free(args_buffer);

    int32_t a = jint(node_result);
    return a;
}
...
Java_com_xiaoyv_flexiflix_extension_javascript_NodeExtension_stringFromJNI(JNIEnv *env,
                                                                           jobject thiz) {
...
Java_com_xiaoyv_flexiflix_extension_javascript_NodeExtension_startNodeWithArguments(JNIEnv *env,
                                                                                    jobject thiz,
                                                                                    jobjectArray arguments) {
    return startNodejs(env, arguments);
}
 ...
 */
    ...
    int node_result = node::Start(argument_count, argv);
//    free(args_buffer);
    __android_log_print(ANDROID_LOG_ERROR, ADBTAG, "exit: %d", node_result);
//    exit(0);
